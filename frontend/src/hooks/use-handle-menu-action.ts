...
export const useHandleMenuAction = () => {
  ...
  const handleAction = async (menuItemValue: string, selectedId: number) => {
    const actionHandlers: {
      ...
      ENABLE_STUDENT_STATUS: () => handleStudentStatus({ userId: selectedId, status: true }).unwrap(),
      DISABLE_STUDENT_STATUS: () => handleStudentStatus({ userId: selectedId, status: false }).unwrap(),
